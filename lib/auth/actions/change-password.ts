...
import { CHANGE_PASSWORD_ERRORS } from '../errorMessages';
...
export async function changePassword(formData: FormData): Promise<AuthResult> {
  ...
  try {
    ...
    if (!user) {
      return { success: false, error: CHANGE_PASSWORD_ERRORS.NOT_AUTHENTICATED };
    }
    ...
    if (!rateLimit.allowed) {
      return { success: false, error: CHANGE_PASSWORD_ERRORS.RATE_LIMITED };
    }
    ...
    return { success: true, message: CHANGE_PASSWORD_ERRORS.SUCCESS };
  } catch (err) {
    console.error('修改密码失败:', err);
    return { success: false, error: CHANGE_PASSWORD_ERRORS.DEFAULT_ERROR };
  }
}
