...
import { LOGIN_HISTORY_ERRORS } from './errorMessages'
...
export async function getLoginHistory(): Promise<GetLoginHistoryResult> {
  try {
    ...
    if (userError || !user) {
      return { success: false, error: LOGIN_HISTORY_ERRORS.NOT_AUTHENTICATED }
    }
    ...
    if (error) {
      ...
      return { success: false, error: LOGIN_HISTORY_ERRORS.FETCH_ERROR }
    }
